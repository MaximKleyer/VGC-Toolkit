...
def _stone_family(pokemon_id: str) -> str:
    """Stone-ownership family: the species root with a trailing gender marker
    (-m/-f) stripped, so a single gendered Mega Stone (e.g. Meowsticite, which
    ships only as the -f variant) validates on either gender of the species."""
...
def validate_member(member: TeamMember, regulation: str = dataio.DEFAULT_REGULATION) -> list[str]:
    ...
    if c.item is not None:
        item = dataio.items().get(_slug(c.item))
        if item is None:
...
def validate_team(team: list[TeamMember], regulation: str = dataio.DEFAULT_REGULATION) -> dict:
    ...
    # Species clause (mega/base/forme collapse together)
    seen = {}
    for i, member in enumerate(team):
        try:
            root = species_root(member.combatant.pokemon_id)
        except KeyError:
            continue
        if root in seen:
...
def export_paste(team: list[TeamMember]) -> str:
    ...
    for member in team:
        c = member.combatant
        mon = dataio.get_pokemon(c.pokemon_id)
        head = showdown_name(c.pokemon_id) + (f" @ {c.item}" if c.item else "")
        lines = [head]
...
def import_paste(text: str) -> list[TeamMember]:
    ...
    for block in re.split(r"\n\s*\n", text.strip()):
        ...
        pokemon_id = _resolve_name(name.strip())
        ...
        for ln in lines[1:]:
            if ln.startswith("- "):
                ...
                pass  # Champions is always level 50
